...
- **隐私优先**：所有数据和操作均在本地处理，无云端依赖

## 主要特性与使用场景

TDXAgent 不仅仅是数据聚合器，更是一个智能分析助手，能够提供深度洞察，以下是一些典型的使用场景：

- **跨平台信息摘要**: 自动从数百条推文、数千条Telegram消息中整理出用户所关注领域（如加密货币、金融市场）的每日要点。
- **热点资产分析**: 针对社交媒体上热议的资产（如 $BTC, $ETH），自动梳理其价格趋势、市场情绪、关键技术指标和社区看法。
- **投资机会识别**: 发现并分析潜在的投资机会（如新的DeFi收益池），并提供其底层逻辑、适用场景、操作步骤和详细的风险提示。
- **安全与风险预警**: 自动识别并警告群聊中的潜在诈骗信息，为您的资产安全提供一层额外的保障。
- **可信的AI分析**: 每个分析结论都引用了真实的数据来源链接，允许您快速回溯原文进行验证，彻底解决大模型内容不可靠（幻觉）的问题。

详情可以见：[使用体验](https://x.com/GT_Chiang/status/1956724605210767528)
...
```bash
git clone https://github.com/guantik-gm/TDXAgent.git
cd TDXAgent
...
| Discord | ✅ 安全 (推荐) | 聊天记录 | 官方数据导出。也提供非官方实时采集模式，但不推荐，存在账户风险。 |  
...
**注意**：为保证账户安全，默认并推荐使用官方的数据导出功能进行分析。项目中也包含了一个非官方的实时数据采集模式（unsafe mode），但它违反了Discord的服务条款，有很高的封号风险，**请不要轻易使用**。
...
尽管系统支持其他 OpenAI 兼容的 API，但强烈推荐使用由 Gemini CLI 驱动的智能体。主要优势在于：
- **免费且强大**: 可以直接免费使用强大的 Gemini 2.5 Pro 模型，其每日免费额度足以满足高强度的分析需求。
- **原生智能体能力**: Gemini CLI 内置了自主工具使用（如Google搜索、网页访问）能力，无需额外开发即可实现信息的交叉验证和深度研究，分析效果远超传统的API调用模式。
- **超大上下文窗口**: 可以一次性处理包含数万条消息的庞大数据集，确保了分析的全面性。
