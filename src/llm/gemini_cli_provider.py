"""
Gemini CLI provider for TDXAgent.

This module provides integration with Gemini CLI, using command-line interface
...
class GeminiCliProvider(BaseLLMProvider):
    ...
    Features:
    - Command-line interface using 'gemini' command
    ...
    def __init__(self, config: Dict[str, Any], data_directory: Optional[str] = None):
        ...
        Args:
            config: Gemini CLI configuration dictionary
        """
        super().__init__(config, "gemini_cli", data_directory)
        ...
        self.model_configs = {
            'default': {
                'context_length': 1000000,  # Gemini 1.5 has large context
                'cost_per_1k_input': 0.00025,  # Gemini Flash pricing
        ...
        self.logger.info("Initialized Gemini CLI provider")
    
    def _validate_cli_availability(self) -> None:
        """Validate that Gemini CLI is available and working."""
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.info(f"Gemini CLI available: {version_info}")
            else:
                ...
                if result.returncode == 0:
                    self.logger.info(f"Gemini CLI available (help command works)")
                else:
                    raise RuntimeError(f"Gemini CLI check failed: {result.stderr}")
                
        except FileNotFoundError:
            raise RuntimeError(f"Gemini CLI not found at path: {self.cli_path}")
        except subprocess.TimeoutExpired:
            raise RuntimeError("Gemini CLI version check timed out")
        except Exception as e:
            raise RuntimeError(f"Failed to validate Gemini CLI: {e}")
    
    async def _make_request(self, request: LLMRequest) -> LLMResponse:
        """
        Make a request to Gemini CLI.
    ...
    async def generate_response(self, prompt: str, model: Optional[str] = None, platform: Optional[str] = None) -> LLMResponse:
        ...
        try:
            ...
            if self._prompt_file_manager:
                ...
                import tempfile
                fd, prompt_file = tempfile.mkstemp(suffix='.txt', prefix='gemini_prompt_')
            ...
            try:
                ...
                self.logger.debug(f"Gemini CLI execution time: {exec_time:.2f}s")
            except asyncio.TimeoutError:
                process.kill()
                await process.wait()
                self.logger.error(f"Gemini CLI timed out after {self.timeout}s")
            ...
            # Check result
            self.logger.debug(f"Gemini CLI return code: {process.returncode}")
            if process.returncode != 0:
                ...
                if error_msg:
                    full_error = f"Gemini CLI stderr: {error_msg}"
                elif stdout_msg:
                    full_error = f"Gemini CLI stdout (non-zero exit): {stdout_msg}"
                else:
                    full_error = f"Gemini CLI exited with code {process.returncode} (no output)"
                
                # Add command info for debugging
                full_error += f" | Command: {cmd_str}"
                
                self.logger.error(f"Gemini CLI failed: {full_error}")
            ...
            self.logger.error(f"Gemini CLI generation failed: {e}")
    ...
    def _build_cli_command(self, prompt_file: str) -> str:
        """
        Build Gemini CLI command string.
        ...
        """
        # Use -y flag for non-interactive mode with input redirection
        cmd_parts = [self.cli_path, '-y']
    ...
    def estimate_tokens(self, text: str) -> int:
        ...
        # Rough estimation: ~3.5 characters per token for mixed content
        # Gemini tends to be more token-efficient than other models
    ...
    def calculate_cost(self, usage: Dict[str, Any]) -> float:
        ...
        input_cost = (input_tokens / 1000) * config.get('cost_per_1k_input', 0.00025)
        output_cost = (output_tokens / 1000) * config.get('cost_per_1k_output', 0.00075)
    ...
    def get_max_context_length(self, model: Optional[str] = None) -> int:
        ...
        """
        return self.model_configs.get(self.default_model, {}).get('context_length', 1000000)
    ...
    def __str__(self) -> str:
        ...
        return f"GeminiCliProvider(cli_path={self.cli_path}, model={self.model})"