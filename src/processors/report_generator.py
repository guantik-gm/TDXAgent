    ...
    def _generate_prompt_files_section(self, platforms_included: List[str], batch_result=None) -> str:
        ...
        try:
            ...
            if prompt_file_paths:
                ...
                sections.append("以上提示词数据文件中包含用于分析的原始数据。可以结合报告中的引用信息，通过命令行工具对这些绝对路径下的文件进行精确的查看、过滤与分析。")
                sections.append("**1. 引用格式详解**:")
                sections.append("""
报告中的引用标签遵循以下格式：

`[<平台标识><批次号?>:<行号><空格><时间戳>]`

*   **`<平台标识>`**: 如 `tg` (Telegram), `tw` (Twitter/X) 等。
*   **`<批次号?>`**: **可选**。当数据分批处理时出现，如 `tg1`, `tg2`。
*   **`:`**: 分隔符。
*   **`<行号>`**: 对应的原始数据行号。
*   **`<空格>`**: 行号后是一个空格，不是闭合括号。
*   **`<时间戳>`**: 消息的日期时间。

**示例**: `[tg:8615]`, `[tg1:7702-8000 时间]`, `[tg:9741 08-29 06:01]`
""")
                sections.append(f"#### 2. 读取与过滤方法")
                sections.append("""
报告中的行号（如 `[tg:9741]`）是**相对于 `<analysis_data>` 标签的**，而非文件的绝对行号。**最佳实践**：先找到数据区的起始行，然后使用 `sed` 进行提取。

**应用示例**：
1.  查找起始行： `grep -n "<analysis_data>" /path/to/your/telegram_analysis.txt`
    （假设返回 `50: <analysis_data>`）
2.  要查找 `[tg:9741]`，目标行号为 `50 + 9741 = 9791`。
3.  执行命令： `sed -n '9791p' /path/to/your/telegram_analysis.txt`
4.  要查找 `[tg:9739-9741]`，则执行：`sed -n '9789,9791p' /path/to/your/telegram_analysis.txt`
""")
